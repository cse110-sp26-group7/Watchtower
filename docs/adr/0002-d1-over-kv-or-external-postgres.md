...
WatchTower is a lightweight observability backend that ingests three kinds of data from instrumented client applications: **events** (thrown errors, performance samples, and user-feedback signals from rating widgets and feedback forms), **deploys** (build/deploy markers used to attribute regressions to a specific deployment), and **projects** (the tenants whose software is being watched). Auth-related tables (users, sessions) are out of scope for this ADR and are addressed in ADR-0005. The backend must run on Cloudflare Workers per the project's technical constraints, which means our persistence layer must be one of Cloudflare's first-party storage products.
...
3. **Write throughput and burst tolerance** — Events arrive in spikes. The store must absorb a realistic burst (low thousands per minute) without backpressuring the Worker into 5xx responses to instrumented clients. We explicitly accept that bursts exceeding the rate limit drop writes (no queue; see Negative Consequences).
4. **Schema flexibility and data lifecycle** — Event payloads will evolve as we add instrumentation. Adding a new field on Tuesday should not require a migration sprint on Wednesday. The store must also support cheap routine deletion for the 30-day retention policy.
...
- The dashboard's required query shapes (date range, GROUP BY, ORDER BY, JOIN across events/deploys/projects) translate directly to standard SQL. Anyone on the team who has taken a databases course can read and review them.
...
- Routine 30-day retention pruning is a single `DELETE WHERE ts < ?` against an indexed column — trivial to implement on a Workers Cron Trigger and bounded in cost.
...
- Each database is capped at **10 GB**; this is not extendable. At our projected event volume this is years of headroom, but a runaway ingestion bug could fill it during a long weekend. The 30-day retention job partially mitigates this in steady state.
...
- **Bursts beyond rate limits drop writes by design.** WatchTower deliberately does not queue ingest. If D1 throttles a write or the ingest rate limiter rejects one, the event is lost (the SDK retries `sendBeacon` once, but no server-side queue catches the difference). Adding a queue (Cloudflare Queues, an R2 spillover bucket, or a buffering Durable Object) would add operational surface area and free-tier pressure we judged not worth it for an academic-scale dashboard. Recorded here so future maintainers don't mistake the absence for an oversight.
...
- The choice does not commit us on the *data model*; ADR-0004 (single events table) defines the polymorphic schema where errors, performance samples, and feedback share one `events` table discriminated by a `type` column, with type-specific fields in a JSON payload column. D1's SQLite engine supports this directly via `json_extract`; the schema decision is independent of this storage decision and can evolve.
...
- **Schema flexibility is good enough** (driver 4). New columns are a single `ALTER TABLE`; storing the raw event payload as a JSON column lets us add new fields without any migration at all, and SQLite's JSON functions (`json_extract`, etc.) let us query into it when needed. This unlocks the polymorphic single-table design adopted in ADR-0004, where errors, performance samples, and feedback share one `events` table discriminated by a `type` column.
- **Cheap retention pruning** (driver 4). The 30-day retention policy is a single `DELETE WHERE ts < ?` against the indexed `ts` column, run on a Workers Cron Trigger. Deletes count against the daily write quota but are bounded in volume by the steady-state event rate.
...
- **Retention pruning is not viable on the free tier** (driver 4): the 1,000 deletes/day cap means we cannot bulk-delete a day's worth of expired events without exhausting the quota. KV is the wrong shape for time-bounded data lifecycle.
...
- **Free `DeleteObject`** (driver 4): retention pruning at the per-object level is operationally free, though listing the keys to delete still costs Class A operations.
...
- **Same SQL semantics as D1** (driver 1/4): a SQLite query that runs in D1 runs identically in a Durable Object's SQLite backend. Retention pruning works the same way (`DELETE WHERE ts < ?`). We could share schema files between the two.
