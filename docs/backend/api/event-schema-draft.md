...
`project_id` is **not** a per-event field. It lives on the batch envelope at `POST /ingest` (`{ "project_id": "wt_…", "events": [...] }`). Format: `wt_` + 8-12 random chars (e.g., `wt_a1b2c3d4`). See `endpoints-draft.md` for envelope spec. Backend storage associates each event row with its envelope's `project_id` on insert.
